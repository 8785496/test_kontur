...
void writeFiles() {
    ...
    children[0].id = 3; strcpy(children[0].name,"Fedya");
    children[1].id = 1; strcpy(children[1].name,"Lena");
    children[2].id = 2; strcpy(children[2].name,"Vasya");
    children[3].id = 0; strcpy(children[3].name,"Vika");
    ...
    sym[7].from = 5; sym[7].to = 0;
    sym[1].from = 5; sym[1].to = 2;

    ofstream fouts("sympathy", ios::binary | ios::out);
    fouts.write((char*)sym, sizeof(sympathy) * 8);
    fouts.close();
    delete[] sym;
}
...
int main() {
    writeFiles();
    cout << "Files 'child' and 'sympathy' created" << endl;
}
